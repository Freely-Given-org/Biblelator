...
ShortProgName = "BibleResourceFrames"
ProgName = "Biblelator Bible Resource Frames"
ProgVersion = "0.15"
...
MAX_CACHED_VERSES = 200 # Per Bible resource window
...
# end of BibleResourceFrames.t



class BibleResourceFrame( ResourceFrame ):
    def __init__( self, parent, master, moduleID ):
        if Globals.debugFlag: print( t("BibleResourceFrame.__init__( {}, {}, {} )").format( parent, master, moduleID ) )
        self.resourceWindowParent, self.myMaster, self.moduleID = parent, master, moduleID
        ResourceFrame.__init__( self, self.resourceWindowParent )
        self.pack()

        if self.resourceWindowParent.contextViewMode=='Default':
            self.resourceWindowParent.contextViewMode = 'BeforeAndAfter'
            self.resourceWindowParent.viewVersesBefore, self.resourceWindowParent.viewVersesAfter = 2, 6
        self.resourceWindowParent.groupCode = GROUP_CODES[0] # Put into first/default BCV group
    ...
    # end of BibleResourceFrame.__init__
    ...
    def getSwordVerseKey( self, verseKey ):
            ...
            return self.myMaster.SwordInterface.makeKey( BBB, C, V )
    # end of BibleResourceFrame.getSwordVerseKey
    ...
    # end of BibleResourceFrame.getVerseData
    ...
    def getCachedVerseData( self, verseKey ):
        ...
        if str(verseKey) in self.cache:
            #if Globals.debugFlag and debuggingThisModule: print( "  " + t("Retrieved from BibleResourceFrame cache") )
        ...
        return verseData
    # end of BibleResourceFrame.getCachedVerseData
    ...
    def displayAppendVerse( self, firstFlag, verseKey, verseDataList, currentVerse=False ):
        ...
        if 0 and Globals.debugFlag and debuggingThisModule:
            print( t("BibleResourceFrame.displayAppendVerse"), firstFlag, verseKey, verseDataList, currentVerse )
            ...
            for entry in verseDataList:
                ...
                if marker and marker[0]=='¬': pass # Ignore end markers for now
                    ...
                    if cleanText:
                        ...
                        lastCharWasSpace = False
                    haveTextFlag = True
                elif marker == 'p#' and self.resourceWindowParent.winType=='DBPBibleResourceWindow':
                    ...
                    logging.critical( t("BibleResourceFrame.displayAppendVerse: Unknown marker {} {} from {}").format( marker, cleanText, verseDataList ) )
    # end of BibleResourceFrame.displayAppendVerse
    ...
    def getBeforeAndAfterBibleData( self, newVerseKey ):
        ...
        """
        if Globals.debugFlag: print( t("BibleResourceFrame.getBeforeAndAfterBibleData( {} )").format( newVerseKey ) )
        ...
        previousVersesData = []
        for n in range( -self.resourceWindowParent.viewVersesBefore, 0 ):
            print( "  getBeforeAndAfterBibleData here with", n, prevIntC, prevIntV )
            if prevIntV > 0: prevIntV -= 1
            elif prevIntC > 0:
                prevIntC -= 1
                prevIntV = self.myMaster.genericBOS.getNumVerses( prevBBB, prevIntC )
                print( " Went back to previous chapter", prevIntC, prevIntV, "from", BBB, C, V )
            else:
                prevBBB = self.myMaster.genericBOS.getPreviousBookCode( BBB )
                prevIntC = self.myMaster.genericBOS.getNumChapters( prevBBB )
                prevIntV = self.myMaster.genericBOS.getNumVerses( prevBBB, prevIntC )
        ...
        nextVersesData = []
        for n in range( 0, self.resourceWindowParent.viewVersesAfter ):
            try: numVerses = self.myMaster.genericBOS.getNumVerses( nextBBB, nextIntC )
    ...
    # end of BibleResourceFrame.getBeforeAndAfterBibleData
    ...
    def updateShownBCV( self, newVerseKey ):
        ...
        if Globals.debugFlag and debuggingThisModule:
            print( "BibleResourceFrame.updateShownBCV( {}) for".format( newVerseKey ), self.moduleID )
            print( "contextViewMode", self.resourceWindowParent.contextViewMode )
        self.clearText() # Leaves the text box enabled
        startingFlag = True

        if self.resourceWindowParent.contextViewMode == 'BeforeAndAfter':
        ...
        elif self.resourceWindowParent.contextViewMode == 'ByVerse':
            self.displayAppendVerse( True, newVerseKey, self.getCachedVerseData( newVerseKey ), currentVerse=True )

        elif self.resourceWindowParent.contextViewMode == 'ByBook':
            BBB, C, V = newVerseKey.getBCV()
            intC, intV = newVerseKey.getChapterNumberInt(), newVerseKey.getVerseNumberInt()
            for thisC in range( 0, self.myMaster.genericBOS.getNumChapters( BBB ) ):
                try: numVerses = self.myMaster.genericBOS.getNumVerses( BBB, thisC )
                except KeyError: numVerses = 0
        ...
        elif self.resourceWindowParent.contextViewMode == 'ByChapter':
            BBB, C, V = newVerseKey.getBCV()
            intV = newVerseKey.getVerseNumberInt()
            try: numVerses = self.myMaster.genericBOS.getNumVerses( BBB, C )
            except KeyError: numVerses = 0
            for thisV in range( 0, numVerses ):
        ...
        self.refreshTitle()
    # end of BibleResourceFrame.updateShownBCV
# end of BibleResourceFrame class



class SwordBibleResourceFrame( BibleResourceFrame ):
    def __init__( self, parent, master, moduleAbbreviation ):
        ...
        self.resourceWindowParent, self.mySwMaster, self.moduleAbbreviation = parent, master, moduleAbbreviation
        BibleResourceFrame.__init__( self, self.resourceWindowParent, master, moduleAbbreviation )
        self.pack()

        #self.SwordModule = None # Loaded later in self.getBeforeAndAfterBibleData()
        self.SwordModule = self.myMaster.SwordInterface.getModule( self.moduleAbbreviation )
        if self.SwordModule is None:
            logging.error( t("SwordBibleResourceFrame.__init__ Unable to open Sword module: {}").format( self.moduleAbbreviation ) )
            self.SwordModule = None
    # end of SwordBibleResourceFrame.__init__


    def refreshTitle( self ):
        self.resourceWindowParent.title( "[{}] {} ({}) {}".format( self.resourceWindowParent.groupCode,
                                    self.moduleAbbreviation, 'Sw' if SwordType=="CrosswireLibrary" else 'SwM',
                                    self.resourceWindowParent.contextViewMode ) )
    # end if SwordBibleResourceFrame.refreshTitle
    ...
    def getVerseData( self, verseKey ):
        ...
        Fetches and returns the internal Bible data for the given reference.
        """
        if Globals.debugFlag: print( t("SwordBibleResourceFrame.getVerseData( {} )").format( verseKey ) )
        if self.SwordModule is not None:
            if verseKey.getChapterNumber()!='0' and verseKey.getVerseNumber()!='0': # not sure how to get introductions, etc.
                ...
                return self.myMaster.SwordInterface.getVerseData( self.SwordModule, SwordKey )
    # end of SwordBibleResourceFrame.getVerseData
# end of SwordBibleResourceFrame class



class DBPBibleResourceFrame( BibleResourceFrame ):
    def __init__( self, parent, master, moduleAbbreviation ):
        ...
        self.resourceWindowParent, self.myDBPMaster, self.moduleAbbreviation = parent, master, moduleAbbreviation
    ...
    def refreshTitle( self ):
        self.resourceWindowParent.title( "[{}] {}.{}{} {}".format( self.resourceWindowParent.groupCode,
                                                ...
                                                self.resourceWindowParent.contextViewMode ) )
    # end if DBPBibleResourceFrame.refreshTitle
    ...
    def getVerseData( self, verseKey ):
        ...
        Fetches and returns the internal Bible data for the given reference.
        """
        if Globals.debugFlag and debuggingThisModule: print( t("DBPBibleResourceFrame.getVerseData( {} )").format( verseKey ) )
        if self.DBPModule is not None:
            if verseKey.getChapterNumber()!='0' and verseKey.getVerseNumber()!='0': # not sure how to get introductions, etc.
                return self.DBPModule.getVerseData( verseKey )
    # end of DBPBibleResourceFrame.getVerseData
# end of DBPBibleResourceFrame class



class InternalBibleResourceFrame( BibleResourceFrame ):
    def __init__( self, parent, master, modulePath ):
        ...
        self.resourceWindowParent, self.myIBMaster, self.modulePath = parent, master, modulePath
        BibleResourceFrame.__init__( self, self.resourceWindowParent, master, modulePath )
    ...
    def refreshTitle( self ):
        self.resourceWindowParent.title( "[{}] {} (InternalBible){} {}".format( self.resourceWindowParent.groupCode,
                        self.modulePath if self.InternalBible is None else self.InternalBible.name,
                        ' NOT FOUND' if self.InternalBible is None else '', self.resourceWindowParent.contextViewMode ) )
    # end if InternalBibleResourceFrame.refreshTitle
    ...
    def getVerseData( self, verseKey ):
        ...
        Fetches and returns the internal Bible data for the given reference.
        """
        if Globals.debugFlag: print( t("InternalBibleResourceFrame.getVerseData( {} )").format( verseKey ) )
        if self.InternalBible is not None:
            try: return self.InternalBible.getVerseData( verseKey )
            except KeyError:
                logging.critical( t("InternalBibleResourceFrame.getVerseData for {} {} got a KeyError!") \
                                                                .format( self.resourceWindowParent.winType, verseKey ) )
    # end of InternalBibleResourceFrame.getVerseData
...
# end of BibleResourceFrames.demo
